...
import { CHART_COLORS, getStockSeries, loadPriceHistory } from "../utils/charts";
import {
    formatCurrency,
    getStockDailyChange,
    getToneFromValue,
...
function Holdings() {
    ...
    const totalValue = activePortfolio.reduce((acc, h) => acc + (Number(h.currentValue) || 0), 0);
    ...
    const sectorDistribution = useMemo(() => {
        ...
        portfolio.forEach((holding) => {
            ...
            bySector[sector] = (bySector[sector] || 0) + Number(holding.currentValue || 0);
        });
    ...
    const bestPerformer = activePortfolio.length
        ? activePortfolio.reduce((prev, current) => (Number(prev.profitLoss) > Number(current.profitLoss) ? prev : current))
        : null;
                ...
                <div className="split-main">
                    <div className="panel holdings-summary">
                        <div className="holdings-summary-top">
                            <div>
                                <span className="eyebrow">Holdings ({activePortfolio.length})</span>
                                <div className="holdings-value">{formatCurrency(totalValue)}</div>
                            </div>
                        </div>
                    ...
                    <div className="panel">
                            ...
                            <table className="data-table dense-table">
                                ...
                                <tbody>
                                    {activePortfolio.map((holding) => {
                                        const investedValue = Number(holding.investedValue) || Number(holding.averageBuyPrice) * Number(holding.quantity);
                                        const plPct = investedValue ? (Number(holding.profitLoss) / investedValue) * 100 : 0;
                                        const alloc = totalValue ? (Number(holding.currentValue) / totalValue) * 100 : 0;
                                        const stock = stocks.find((item) => item.id === holding.stockId) || holding;
                                        const daily = getStockDailyChange(stock);
                                        const series = getStockSeries(holding, priceHistoryById, 12);

                                        return (
                                            <tr key={holding.stockId || holding.symbol}>
                                                <td>
                                                    <div className="ticker-cell">
                                                        <strong>{holding.companyName || holding.symbol}</strong>
                                                        <span>
                                                            {holding.quantity} shares · Avg {formatCurrency(holding.averageBuyPrice)}
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="align-right">
                                                    ...
                                                    <div className={`money-${getToneFromValue(daily.percent)}`}>
                                                        {daily.percent >= 0 ? "+" : ""}
                                                        {daily.percent.toFixed(2)}%
                                                    </div>
                                                </td>
                                                <td>
                                                    <Sparkline points={series} positive={daily.percent >= 0} />
                                                </td>
                                                <td className={`align-right money-${getToneFromValue(plPct)}`}>
                                                    <div>{holding.profitLoss >= 0 ? "+" : ""}{formatCurrency(holding.profitLoss)}</div>
                                                    <div>{plPct >= 0 ? "+" : ""}{plPct.toFixed(2)}%</div>
                                                </td>
                                                <td className="align-right">
                                                    <div>{formatCurrency(holding.currentValue)}</div>
                                                    <div className="muted">{formatCurrency(investedValue)}</div>
                                                </td>
                                            </tr>
                                        );
                                    })}
                        ...
                        {!activePortfolio.length && (
                            <div className="table-state">
                                {isLoading || isPortfolioLoading ? "Loading holdings..." : "No active holdings found."}
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {worstPerformer ? (
                        <div className="panel performer-card negative">
                            ...
                            <div className="performer-row">
                                ...
                                <div className="money-negative">{formatCurrency(worstPerformer.profitLoss)}</div>
                            </div>
                        </div>
                    ) : null}
                </aside>
